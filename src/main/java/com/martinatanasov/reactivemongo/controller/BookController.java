...
public class BookController {
    ...
    @PostMapping("/")
    public Mono<ResponseEntity<Void>> addBook(@Valid @RequestBody BookDTO bookDTO) {
        return bookService.saveBook(bookDTO)
                //Return 200 if the resource is updated and the URL for the object
                .map(data -> ResponseEntity.created(
                        UriComponentsBuilder.fromHttpUrl("http://localhost:5000/api/v1/books/" + data.getId()).build().toUri()
                ).build());
    }

    @PutMapping("/{id}")
    public Mono<ResponseEntity<Void>> updateBook(@PathVariable final String id,
                                                 @Valid @RequestBody BookDTO bookDTO) {
        return bookService.updateBook(id, bookDTO)
                //Return 404 if book with this ID is not found
                .switchIfEmpty(Mono.error(new ResponseStatusException(HttpStatus.NOT_FOUND)))
                //Return 200 if the resource is updated
                .map(data -> ResponseEntity.ok().build());
    }

    @PatchMapping("/{id}")
    public Mono<ResponseEntity<Void>> patchBook(@PathVariable final String id,
                                                @Valid @RequestBody BookDTO bookDTO) {
        return bookService.patchBook(id, bookDTO)
                .switchIfEmpty(Mono.error(new ResponseStatusException(HttpStatus.NOT_FOUND)))
                //Return 204 if the book is updated
